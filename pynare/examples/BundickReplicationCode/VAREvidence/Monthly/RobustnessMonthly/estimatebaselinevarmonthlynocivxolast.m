...
enddate = length(date);
%% DEFINE VARIABLES
vardata = [logoutput logconsumption logdurableconsumption loghours logprices logm2 shadowrate logvxo];
...
subplot(2,3,1)
hold on
plot(t,100*IRpointquarterly(:,1),'*','MarkerEdgeColor','b','MarkerFaceColor','b','MarkerSize',9)
...
subplot(2,3,2)
hold on
plot(t,100*IRpointquarterly(:,2),'*','MarkerEdgeColor','b','MarkerFaceColor','b','MarkerSize',9)
plot(t,zeros(irflength,1),'--','Color','k','LineWidth',1,'HandleVisibility','off')
title('Consumption','FontSize',16);xlim([t(1) t(end)]);set(gca,'XTick',t(4:4:end),'FontSize',12);

subplot(2,3,4)
hold on
plot(t,100*IRpointquarterly(:,3),'*','MarkerEdgeColor','b','MarkerFaceColor','b','MarkerSize',9)
plot(t,zeros(irflength,1),'--','Color','k','LineWidth',1,'HandleVisibility','off')
title('Hours','FontSize',16);xlim([t(1) t(end)]);set(gca,'XTick',t(4:4:end),'FontSize',12);

subplot(2,3,5)
hold on
plot(t,100*IRpointquarterly(:,8),'*','MarkerEdgeColor','b','MarkerFaceColor','b','MarkerSize',9)
plot(t,zeros(irflength,1),'--','Color','k','LineWidth',1,'HandleVisibility','off')
