...
stoch_simul(order=3,periods=10000,irf=20,replic=2000,noprint,nograph,nomoments,nofunctions,nocorr,pruning);

y_evola			= y_evola./mean(y);
c_evola			= c_evola./mean(c);
inv_evola		= inv_evola./mean(inv);
n_evola			= n_evola./mean(n);
vola_evola		= vola_evola./mean(vola);
vxo 			= 100*sqrt(4*(max(varexpre,1e-8)));
meanvxo 		= 100*sqrt(4*mean(varexpre));
vxo_evola 		= 100*sqrt(4*(max(varexpre_evola+mean(varexpre),1e-8)));
logvxo_evola	= log(vxo_evola./meanvxo);
...
subplot(2,3,1)
hold on
plot(t,100*y_evola,'--','Color','r','LineWidth',3)
plot(zeros(options_.irf,1),'k--','HandleVisibility','off'); xlim([0 options_.irf]);set(gca,'XTick',[4:4:options_.irf],'FontSize',12);
...
subplot(2,3,2)
hold on
plot(t,100*c_evola,'--','Color','r','LineWidth',3)
plot(zeros(options_.irf,1),'k--','HandleVisibility','off'); xlim([0 options_.irf]);set(gca,'XTick',[4:4:options_.irf],'FontSize',12);
ylabel('Percent','FontSize',12)
...
subplot(2,3,3)
hold on
plot(t,100*inv_evola,'--','Color','r','LineWidth',3)
plot(zeros(options_.irf,1),'k--','HandleVisibility','off'); xlim([0 options_.irf]);set(gca,'XTick',[4:4:options_.irf],'FontSize',12);
ylabel('Percent','FontSize',12)
...
subplot(2,3,4)
hold on
plot(t,100*n_evola,'--','Color','r','LineWidth',3)
plot(zeros(options_.irf,1),'k--','HandleVisibility','off'); xlim([0 options_.irf]);set(gca,'XTick',[4:4:options_.irf],'FontSize',12);
ylabel('Percent','FontSize',12)
...
subplot(2,3,5)
hold on
plot(100*logvxo_evola,'--','Color','r','LineWidth',3)
plot(zeros(options_.irf,1),'k--','HandleVisibility','off'); xlim([0 options_.irf]);set(gca,'XTick',[4:4:options_.irf],'FontSize',12);
ylabel('Percent','FontSize',12)
...
legend(sprintf('Traditional Impulse Response\nStochastic Steady State'),sprintf('Unconditional Generalized\nImpulse Response'),'Location','NorthEast')
