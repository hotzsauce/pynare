...
for i = 1:nperiods
   ...
   if i == 1
        ...
        if i == shockperiod
            ...
            ashock(i,1) = 0.937;%rhoa*ashock(i-1,1) + (1-rhoa)*ass + lagvolashock(i,1)*epsilonashock(i,1);
            
            epsilonvolashock(i,1) = 3;
			...
			abaseline(i,1) = 0.937;%rhoa*abaseline(i-1,1) + (1-rhoa)*ass + lagvolabaseline(i,1)*epsilonabaseline(i,1);
...
ylim([-0.8 0.2]);set(gca,'YTick',[-0.8:0.2:0.2],'FontSize',12);ylabel('Percent','FontSize',12)
...
ylim([-0.6 0.1]);set(gca,'YTick',[-0.6:0.1:0.1],'FontSize',12);ylabel('Percent','FontSize',12)
...
ylim([-1.5 1]);set(gca,'YTick',[-1.5:0.5:1.0],'FontSize',12);ylabel('Percent','FontSize',12)
...
legend('Optimal Policy Rule',sprintf('Taylor Rule\nAway From Zero Lower Bound'),sprintf('Taylor Rule\nSix-Quarter Zero Lower Bound Episode'),'Location','NorthEast')

set(gcf, 'PaperPositionMode', 'manual');
set(gcf, 'PaperUnits', 'inches');
set(gcf, 'PaperOrientation', 'landscape');
set(gcf, 'PaperPosition', [-0.75 -.65 12.0 9.25]);
print('-dpdf',['RoleMonetaryPolicy.pdf'])
