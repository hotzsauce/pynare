...
save baselinevarestimation.mat
...
print('-dpdf',['BaselineVAR.pdf'])


% Save VXO IRF for matching with model

% Adding inflation and monetary policy

irfdata = vec([IRpoint(:,1:6,1) IRpoint(:,8,1)] );

weights = inv(diag( vec([IRstd(:,1:6,1) IRstd(:,8,1)]) ).^(2));

save irfdata.mat irfdata weights IRpoint IRlo_1 IRup_1 IRstd
