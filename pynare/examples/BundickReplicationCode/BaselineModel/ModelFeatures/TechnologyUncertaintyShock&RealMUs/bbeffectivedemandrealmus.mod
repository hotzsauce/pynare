...
var a, c, de, deltau, deltauprime, expre, expre2, expvf1sigma, inv, k, mu, n,
	pe, pie, profit, q, r, rr, rrk, sdf, u, varexpre, vf, vola, volz, w, y, z;

varexo ea, evola, evolz, ez;

parameters 	alpha, beta, delta0, delta1, delta2, eta, muelasticity, phik, 
			sigma, ies, frischlse, thetavf, thetamu, leverageratio, fixedcost,
			rhor, rhopie, rhoy,	rhoa, rhovola, volvola, rhoz, rhovolz, volvolz, productionconstant, utilityconstant, 
...
z = (1 - rhoz)*zss + rhoz*z(-1) + volz(-1)*ez;
volz = rhovolz*volz(-1) + (1 - rhovolz)*volzss + volvolz*evolz;
end;
...
vxo_ea	 		= 100*sqrt(4*(max(varexpre_ea+varexpre_sss,1e-16)));
vxo_ez 			= 100*sqrt(4*(max(varexpre_ez++varexpre_sss,1e-16)));
...
subplot(3,3,6)
hold on
plot(t,100*w_evola./w_sss,'-.','Color',[0 0.8 0],'LineWidth',3)
plot(zeros(irflength,1),'k--','HandleVisibility','off'); xlim([0 irflength]);set(gca,'XTick',[4:4:irflength],'FontSize',12);
...
subplot(3,3,7)
hold on
plot(t,400*rr_evola,'-.','Color',[0 0.8 0],'LineWidth',3)
plot(zeros(irflength,1),'k--','HandleVisibility','off'); xlim([0 irflength]);set(gca,'XTick',[4:4:irflength],'FontSize',12);
title('Real Interest Rate','FontSize',16);xlim([t(1) t(end)]);set(gca,'XTick',t(4:4:end),'FontSize',12);
%ylim([-0.3 0.1]);set(gca,'YTick',[-0.3:0.1:0.1],'FontSize',12);ylabel('Percentage Points','FontSize',12)

figure(1)
subplot(3,3,8)
hold on
plot(t,100*vola_evola./vola_sss,'-.','Color',[0 0.8 0],'LineWidth',3)
plot(zeros(irflength,1),'k--','HandleVisibility','off'); xlim([0 irflength]);set(gca,'XTick',[4:4:irflength],'FontSize',12);
title('Shock Volatility','FontSize',16);xlim([t(1) t(end)]);set(gca,'XTick',t(4:4:end),'FontSize',12);
ylim([0 100]);set(gca,'YTick',[0:20:100],'FontSize',12);ylabel('Percent','FontSize',12)
