...
class DynareSemanticAnalyzer(ABCVisitor):
	...
	def visit_ShocksBlock(self, node):
		...
		# self.visit(boundaries.arguments)
		for single_shock in node._list:
			var_name = single_shock.shock_var.value
			var_symbol = self.current_scope.lookup(var_name)

			if var_symbol.stype.name not in (dyn.VAREXO, dyn.VAREXO_DET):
				token = single_shock.shock_var.token
				raise TypeError(
					f'row {token.line_number}, column {token.line_pos}. '
					f'variable {repr(var_name)} was not declared as exogenous. '
					'Cannot assign as a shock'
				)

			if single_shock.shock_type == 'variance':
				self.visit(single_shock.variance)

			elif single_shock.shock_type == 'stderr':
				self.visit(single_shock.stderr)

			else:
				raise NotImplementedError(f'{single_shock.shock_type} shock semantics')
