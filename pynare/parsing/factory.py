...
class DynareModelFactory(base.ABCVisitor):
	...
	def visit_ShocksBlock(self, node):
		...
		shocks_only_variance_for_now = dict()
		...
		for single_shock in node._list:
			...
			if single_shock.shock_type == 'variance':
				...
				raise NotImplementedError(f'{single_shock.shock_type} shock factory')


			shocks_only_variance_for_now[var_name] = variance

		self.outline._shocks = shocks_only_variance_for_now
