...
var a, c, de, deltau, deltauprime, expre, expre2, expvf1sigma, inv, k, mu, n,
	pe, pie, profit, q, r, rr, rrk, sdf, u, varexpre, vf, vola, w, y, z, invlaborshare;
...
stoch_simul(order=3,periods=2500500,irf=20,replic=2000,noprint,nograph,nomoments,nofunctions,nocorr,pruning);

y_evola				= y_evola./mean(y);
mu_evola			= mu_evola./mean(mu);
invlaborshare_evola	= invlaborshare_evola./mean(invlaborshare);

vxo 				= 100*sqrt(4*(max(varexpre,0.000026)));
meanvxo 			= 100*sqrt(4*mean(varexpre));
vxo_evola 			= 100*sqrt(4*(max(varexpre_evola+mean(varexpre),0.000026)));
...
subplot(2,3,3)
hold on
plot(100*logvxo_evola,'o','MarkerEdgeColor','k','MarkerFaceColor','k','MarkerSize',7)
plot(zeros(options_.irf,1),'k--','HandleVisibility','off'); xlim([0 options_.irf]);set(gca,'XTick',[4:4:options_.irf],'FontSize',12);
ylabel('Percent','FontSize',12)
...
subplot(2,3,2)
hold on
plot(t,100*mu_evola,'o','MarkerEdgeColor','k','MarkerFaceColor','k','MarkerSize',7)
plot(zeros(options_.irf,1),'k--','HandleVisibility','off'); xlim([0 options_.irf]);set(gca,'XTick',[4:4:options_.irf],'FontSize',12);
ylabel('Percent','FontSize',12)
title('Inverse Labor Share','FontSize',16);xlim([t(1) t(end)]);set(gca,'XTick',t(4:4:end),'FontSize',12);ylabel('Percent','FontSize',12)
