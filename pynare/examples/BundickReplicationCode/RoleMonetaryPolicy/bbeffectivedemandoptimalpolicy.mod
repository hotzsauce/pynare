...
var a, c, de, deltau, deltauprime, expre, expre2, expvf1sigma, inv, k, mu, n,
	pe, pie, profit, q, r, rr, rrk, sdf, u, varexpre, vf, vola, w, y, 
	
	cfp, defp, deltaufp, deltauprimefp, expvf1sigmafp, invfp, kfp, mufp, nfp,  
	pefp, piefp, qfp, rfp, rrfp, rrkfp, sdffp, ufp, vffp, wfp, yfp;
	
varexo ea, evola;

parameters 	alpha, beta, delta0, delta1, delta2, eta, phip, phik, 
			sigma, ies, frischlse, thetavf, thetamu, leverageratio, fixedcost,
			rhopie, rhox, rhoa, rhovola, volvola, productionconstant, utilityconstant, 
            ass, css, dess, deltauss, deltauprimess, express, expre2ss, expvf1sigmass, invss, kss, nss, muss, 
            pess, piess, profitss, qss, rss, rrss, rrkss, sdfss, uss, varexpress, vfss, volass, wss, yss;
...
log(r) = log(rrfp) + log(piess) + rhopie*log(pie/piess) + rhox*log(y/yfp);
...
yfp + fixedcost 			= productionconstant*nfp^(1 - alpha)*(ufp*kfp(-1))^(alpha);
cfp + leverageratio*kfp/rrfp 	= wfp*nfp  + defp + leverageratio*kfp(-1);
wfp 					= ((1 - eta)/eta)*cfp/(1 - nfp);
vffp 					= (utilityconstant*a*(cfp^(eta)*(1 - nfp)^(1 - eta))^((1 - sigma)/thetavf)  + beta*expvf1sigma^(1/thetavf))^(thetavf/(1 - sigma));
expvf1sigmafp 			= vffp(+1)^(1 - sigma);
wfp*nfp 				= (1 - alpha)*(yfp + fixedcost)/mufp;
rrkfp*ufp*kfp(-1) 		= alpha*(yfp + fixedcost)/mufp;
qfp*deltauprimefp*ufp*kfp(-1) = alpha*(yfp + fixedcost)/mufp;
kfp 			= ((1 - deltaufp) - (phik/2)*(invfp/kfp(-1) - delta0)^(2))*kfp(-1) + invfp;
deltaufp		= delta0 + delta1*(ufp-1) + (delta0/2)*(ufp-1)^(2);
deltauprimefp	= delta1 + delta2*(ufp-1);
sdffp = beta*(a/a(-1))*((cfp^(eta)*(1 - nfp)^(1 - eta))/(cfp(-1)^(eta)*(1 - nfp(-1))^(1 - eta)))^((1 - sigma)/thetavf)
			*(cfp(-1)/cfp)*(vffp^(1 - sigma)/expvf1sigmafp(-1))^(1 - 1/thetavf);
1 = rrfp*sdffp(+1);
1 = rfp*sdffp(+1)*(piefp(+1))^(-1);
1 = sdffp(+1)*(defp(+1) + pefp(+1))/pefp;
log(rfp) = log(rss) + rhopie*log(piefp/piess);
defp = yfp - wfp*nfp - invfp - leverageratio*(kfp(-1) - kfp/rrfp);
1 = sdffp(+1)*(ufp(+1)*rrkfp(+1) + 
     qfp(+1)*((1 - deltaufp(+1)) - (phik/2)*(invfp(+1)/kfp - delta0)^(2) + phik*(invfp(+1)/kfp - delta0)*(invfp(+1)/kfp)))/qfp;
1/qfp = 1 - phik*(invfp/kfp(-1) - delta0);
0 = (1 - thetamu) + thetamu/mufp ;
end;
...
defp 	 		= 	dess;
deltaufp 		= 	delta0;
deltauprimefp 	= 	delta1;
expvf1sigmafp 	= 	expvf1sigmass;
invfp 	 		= 	invss;
kfp 	 		= 	kss;
nfp 	 		= 	nss;
mufp 	 		= 	muss;
...
subplot(2,3,1)
hold on
plot(t,100*y_evola./y_sss,'-.','Color',[0 0.8 0],'LineWidth',3)
...
subplot(2,3,2)
hold on
plot(t,100*c_evola./c_sss,'-.','Color',[0 0.8 0],'LineWidth',3)
plot(zeros(irflength,1),'k--','HandleVisibility','off'); xlim([0 irflength]);set(gca,'XTick',[4:4:irflength],'FontSize',12);
...
subplot(2,3,3)
hold on
plot(t,100*inv_evola./inv_sss,'-.','Color',[0 0.8 0],'LineWidth',3)
plot(zeros(irflength,1),'k--','HandleVisibility','off'); xlim([0 irflength]);set(gca,'XTick',[4:4:irflength],'FontSize',12);
...
subplot(2,3,4)
hold on
plot(t,100*mu_evola./mu_sss,'-.','Color',[0 0.8 0],'LineWidth',3)
plot(zeros(irflength,1),'k--','HandleVisibility','off'); xlim([0 irflength]);set(gca,'XTick',[4:4:irflength],'FontSize',12);
...
subplot(2,3,5)
hold on
plot(t,100*n_evola./n_sss,'-.','Color',[0 0.8 0],'LineWidth',3)
plot(zeros(irflength,1),'k--','HandleVisibility','off'); xlim([0 irflength]);set(gca,'XTick',[4:4:irflength],'FontSize',12);
