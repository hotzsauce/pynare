...
class _ASTWriter(ABCVisitor):
	...
	def add(self, new_string):
		# so we don't have to see "self.text = self.text + x" all over
	...
	def visit_Var(self, node):
		var_name = node.value
		var_value = self.scope.lookup(var_name)

		if var_value is None:
			self.args.append(node.value)
			self.add(node.value)
		else:
			self.add_number(var_value)
