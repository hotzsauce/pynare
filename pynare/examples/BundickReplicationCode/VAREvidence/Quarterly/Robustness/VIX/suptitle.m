function hout=suptitle(str,options)
%SUPTITLE Puts a title above all subplots.
%	SUPTITLE('text') adds text to the top of the figure
%	above all subplots (a "super title"). Use this function
%	after all subplot commands.
%	options(1,1) = fontsize
%	options(2,1) = perc allocated to subfigure
...
%ORIGINAL CODE = %fs = get(gcf,'defaultaxesfontsize')+4;
fs = 24;
else
plotregion = options(2,1);
%
fs	       = options(1,1);
end;
%%
...
ht=text(.5,titleypos-1,str);set(ht,'horizontalalignment','center','fontsize',fs,'Interpreter','latex');
set(gcf,'nextplot',np);
axes(haold);
if nargout,
	hout=ht;
